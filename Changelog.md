...
- 优化：直播内容在系统“正在播放”中的标题、主播、分区与封面显示
- 优化：视频缓冲时的加载提示与弹幕状态同步
- 优化：直播弹幕的连接与鉴权稳定性
- 优化：网络请求及设备标识逻辑，提升播放、登录和直播服务的稳定性
- 优化：多设备账号同步逻辑，减少不必要的同步操作
- 优化：更新后 AI 字幕默认状态将重置为关闭，如有需要可在设置中重新开启
- 修复：多设备同步账号时可能出现的重复账号、账号冲突或登录状态未及时更新问题
- 修复：番剧推荐页的横向间距、Banner 封面加载异常问题
- 修复：排行榜切换地区时的焦点与列表显示问题

# Miao Project Version 2.5.0 Update Notes
- 新增：弹幕聚合
- 新增：设置 - 弹幕密度控制
- 新增：按下 Tab 刷新当前页面
- 新增：强制信号输出 - 帧率
- 优化：弹幕加载速度
- 优化：番剧自动跳过 OP / ED 的情况下，仍显示跳过按钮
- 优化：移除「匹配视频动态范围和帧率」
- 优化：HDR 播放失败后，降级到 SDR 视频流
- 优化：视频播放器稳定性
- 修复：视频收藏夹可能导致 App 崩溃
- 修复：搜索页面可能会发生崩溃
- 修复：弹幕设置透明度后，描边显示在弹幕内部
...
- 新增：番剧跳过区域渲染在进度条上
- 新增：番剧禁止自动跳过 OP / ED 后，显示手动跳过按钮
- 新增：视频详情页 - 添加到稍后再看
- 新增：直播 - 优化加载 HEVC
- 新增：弹幕 - 按类型屏蔽
- 修复：VLC 直播播放器可能导致 App 卡死
- 修复：番剧 - 部份 Banner 错误显示为不支持的类型
- 修复：屏蔽证书报错的CDN
- 优化：视频调试界面 - CPU占用 / 内存占用 / 温度状态
- 优化：优先加载 8K 视频
- 优化：解析视频过程中的内存占用

# Miao Project Version 2.4.6 Update Notes
- 重构：视频加载播放模块
- 优化：播放视频加载速度
- 优化：播放视频过程中的数据读取安全性
- 优化：视频调试界面显示正在播放的视频链接
- 优化：充电专属视频 - 视频详情页简介部份优先展示
- 优化：充电专属视频 - 未付费的情况下，点击播放弹出提示
- 优化：视频弹幕默认字号改为 42 pt
- 修复：部分用户动态页面解析错误
- 修复：用户因单一CDN无法加载导致无法播放
- 修复：特殊情况下，播放视频跳过第一帧加载

# Miao Project Version 2.4.5 Update Notes
- 修复：部分用户动态页面解析错误
